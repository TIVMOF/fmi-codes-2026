...
export default function SimulationsPage() {
	...
	const filteredSimulations = simulations.filter((s) => s.name.toLowerCase().includes(searchQuery.toLowerCase()));
		...
		<div className='mx-auto max-w-7xl p-6'>
			<div className='mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
				<div>
					...
					<p className='mt-2 text-muted-foreground'>All your emergency evacuation simulations</p>
				</div>
				<Button asChild>
			...
			{!isLoading && simulations.length > 0 && (
				<div className='mb-6 max-w-sm'>
					<div className='relative'>
						...
						<Input placeholder='Search simulations...' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className='bg-input pl-10' />
					</div>
				</div>
			)}
			...
			{isLoading ? (
				...
				<Card className='border-dashed border-border bg-card'>
					<CardContent className='flex flex-col items-center justify-center py-12'>
						...
						<p className='mt-2 text-center text-sm text-muted-foreground'>Create your first emergency evacuation simulation to get started</p>
